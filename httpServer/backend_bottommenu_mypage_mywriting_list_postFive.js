...
export default function myListMyWritingFive(request, response) {
  
 if (request.url === '/Five') { // else문 수정
  let body = "";
  ...
  request.on("end", function () {
    ...
    conn.connect();
    conn.query(
      `select cm_id, cm_detail from cm_post where cm_id='${jwtunlockId}' order by '${text}' desc limit 5`,
      function (err, data) {
        if (err) throw err;
        else {
          console.log(data);
