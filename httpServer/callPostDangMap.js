...
export default function callPostDangMap(request, response) {
  if (request.url.startsWith("/menuMap")) { //댕맵의 지도에 발자국 전부 띄우는 코드
    ...
    request.on("end", function () {
      ...
      for (const key in cooData) {
        ...
        let targetId = jsonwebtoken.jwtCheck(cooData[key][2]).id
        let conn = mysql.createConnection(cmServer.mysqlInfo);
        conn.connect();
        conn.query(
          `insert into map_tables(latitude, longitude, id) values(${cooData[key][0]}, ${cooData[key][1]}, '${targetId}')`,
          function (err) {
            if (err) throw err;
            else {
  ...
  if(request.url.startsWith('/dragMarker')) { // 댕맵에서 마커 드래그가 끝났을 때
    ...
    request.on("end", function(){ // 전송된 데이터를 다 받은 후
      ...
      let targetId = jsonwebtoken.jwtCheck(dragData[key][2]).id;
      response.writeHead(200, {'Content-Type': 'text/html'});
      response.end();
      for(const key in dragData){
        ...
        conn.query(`UPDATE map_tables SET latitude = '${dragData[key][0]}', longitude = '${dragData[key][1]}' WHERE id = '${targetId}' and latitude = '${dragData[key][3]}' and longitude = '${dragData[key][4]}'`,
        function(err){
          if(err) throw err;
