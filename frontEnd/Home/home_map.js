...
let imageSrc = 'https://i.ibb.co/8rzCbPc/dogpaw.png';
let otImageSrc = 'https://i.ibb.co/bQkqT8r/ot-dogpaw.png';
...
function loadMarker(callback) {
  ...
  const cookieId = document.cookie.split("=")[1].split(";")[0];
  xhr.open("GET", `http://ec2-3-37-160-130.ap-northeast-2.compute.amazonaws.com/loadMap?id=${cookieId}`);
  // httpRequest.send(`re1=${result[0]}`);
  xhr.send();
  ...
  function wholeMarker(callback){
    ...
    const xhr = new XMLHttpRequest();
    const cookieId = document.cookie.split("=")[1].split(";")[0];
    xhr.open("GET", `http://ec2-3-37-160-130.ap-northeast-2.compute.amazonaws.com/wholeFootprint?id=${cookieId}`);
    // httpRequest.send(`re1=${result[0]}`);
    xhr.send(); 
    xhr.addEventListener('load', function(){
