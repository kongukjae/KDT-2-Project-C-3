function dangstarLike(postIndex, index, writerNickname) {
  ...
  const _URL = `http://13.124.220.4:2080/likeCheck`;
  const likeURL = "http://13.124.220.4:2080/dangstarLike";

  likeXhr.open("POST", _URL, true);
  likeXhr.send(
    `writerNickname=${writerNickname}&postLikeIdx=${postIndex}&cookie=${cookie}`
  );
  ...
  like.addEventListener("click", function () {
    ...
    // xhr.open("POST", `http://13.124.220.4:2080/postBoard/postBoardLike`, true);
