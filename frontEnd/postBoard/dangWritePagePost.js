function keepDiary() {
  ...
  yastContentInput.setAttribute(
    "style",
    "width: 500px; height: 40px; resize: none; border: 3px solid #F7786B; border-radius: 9px; font-size: 20px"
  );
  ...
  postContentInput.setAttribute(
    "style",
    "width: 500px; height: 400px;resize: none; border: 3px solid #F7786B; border-radius: 9px; font-size: 20px"
  );
  ...
  async function uploadPost(type) {
    ...
    if (type === "dangStar") {
      ...
      body = {
        jwt: token,
        mainText: postContentInput.value,
        imageType: imageType,
      };
    } else {
      ...
      body = {
        jwt: token,
        titleText: yastContentInput.value,
        mainText: postContentInput.value,
        imageType: imageType,
      };
    }
    await fetch(`http://13.124.220.4:2080/${type}WriteSubmit`, {
      method: "POST",
    ...
    await fetch(`http://13.124.220.4:2080/${type}ImageSubmit`, {
      method: "POST",
      body: imageFormData,
    })
      .then((res) => res)
