 ...
 // 지도를 생성한다
    ...
    if(addFlag){
      ...
      if (overlayChecker === false) {
        ...
        if(textCount < 10 && textCount >= 0) {
          ...
          countFootprintCount.innerText = textCount;
          httpRequest.open("POST", `http://43.201.52.54:2080/menuMap`, true);
  ...
  function addMarker(position) {
    ...
    kakao.maps.event.addListener(marker, "dragend", function () {
      ...
      const httpRequest = new XMLHttpRequest();
      httpRequest.open("POST", `http://43.201.52.54:2080/dragMarker`, true);
  ...
  function allMarker(callback, type) {
    return new Promise(function (resolve, reject) {
      fetch(`http://43.201.52.54:2080/${getURL[type]}?id=${targetId}`)
...
makeControlBtnsArr[0].addEventListener("click", () => {
  ...
  finishBtn.addEventListener('click',()=>{
    for(let i of deleteTarget){
      ...
      xhr.open("POST", `http://43.201.52.54:2080/mapDelete`);
...
makeControlBtnsArr[2].addEventListener("click", () => {
  ...
  async function publicTalkAsync(){
    let publicTalkList = await fetch('http://43.201.52.54:2080/publicChatLocation')
