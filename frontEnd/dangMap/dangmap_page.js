 ...
 map.setZoomable(false);
    ...
    if(addFlag){
      ...
      if (overlayChecker === false) {
        ...
        if(textCount < 10 && textCount >= 0) {
          ...
          countFootprintCount.innerText = textCount;
          httpRequest.open("POST", `http://ec2-3-37-160-130.ap-northeast-2.compute.amazonaws.com/menuMap`, true);
  ...
  function addMarker(position) {
    ...
    kakao.maps.event.addListener(marker, "dragend", function () {
      ...
      const httpRequest = new XMLHttpRequest();
      httpRequest.open("POST", `http://ec2-3-37-160-130.ap-northeast-2.compute.amazonaws.com/dragMarker`, true);
  ...
  const markersImage = {
    0: "https://i.ibb.co/SQYm56Y/fr-dogpaw.png",
    1: "https://i.ibb.co/2WJK1m0/star-dogpaw.png",
    2: "https://i.ibb.co/bQkqT8r/ot-dogpaw.png",
    3 : "https://i.ibb.co/8rzCbPc/dogpaw.png"}
  const getURL = {
    0: "frFootprint",
    1: "starFootprint",
    2: "otFootprint",
  ...
  function allMarker(callback, type) {
    return new Promise(function (resolve, reject) {
      fetch(`http://ec2-3-37-160-130.ap-northeast-2.compute.amazonaws.com/${getURL[type]}?id=${targetId}`)
...
makeControlBtnsArr[0].addEventListener("click", () => {
  ...
  finishBtn.addEventListener('click',()=>{
    for(let i of deleteTarget){
      ...
      xhr.open("POST", `http://ec2-3-37-160-130.ap-northeast-2.compute.amazonaws.com/mapDelete`);
...
makeControlBtnsArr[2].addEventListener("click", () => {
  ...
  async function publicTalkAsync(){
    let publicTalkList = await fetch('http://ec2-3-37-160-130.ap-northeast-2.compute.amazonaws.com/publicChatLocation')
