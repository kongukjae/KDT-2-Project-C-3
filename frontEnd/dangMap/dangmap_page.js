...
let markersObject = {
  ...
  set appendMarker(value) {
    ...
    if (
      value[1] === 3 &&
      value[3].getFullYear() === todayNow.getFullYear() &&
      value[3].getMonth() === todayNow.getMonth() &&
      value[3].getDate() === todayNow.getDate()
    ) {
      this.todayCount++;
    }
...
function map() {
    ...
    if (addFlag) {
      ...
      if (overlayChecker === false) {
        ...
        if (textCount < 10 && textCount >= 0) {
          ...
          countFootprintCount.innerText = textCount;
          httpRequest.open("POST", `http://13.124.220.4:2080/menuMap`, true);
  ...
  function addMarker(position) {
    ...
    kakao.maps.event.addListener(marker, "dragend", function () {
      ...
      const httpRequest = new XMLHttpRequest();
      httpRequest.open("POST", `http://13.124.220.4:2080/dragMarker`, true);
  ...
  function allMarker(callback, type) {
    return new Promise(function (resolve, reject) {
      fetch(`http://13.124.220.4:2080/${getURL[type]}?id=${targetId}`)
        ...
        .then((result) => {
          ...
          for (const key in result) {
            ...
            markersObject.appendMarker = [
              result[key][2],
              type,
              markerNow,
              new Date(result[key][3]),
            ];
            markersObject.appendPosition = {
              lat: result[key][0],
              lng: result[key][1],
            };
            createOverlay(
  ...
  function countMarkersDate() {
    ...
    for (i in markersTodayData) {
      if (new Date(markersTodayData[i][1]).getDate() !== startToday.getDate()) {
        ...
        new Date(markersTodayData[i][1]).getDate() === startToday.getDate()
      ) {
        markersDateCount -= 1;
        console.log(markersDateCount);
      }
  ...
  async function getMarkersObject() {
    ...
    makeControlBtnsArr[1].addEventListener("click", () => {
      ...
      let target = [];
      for (let i = 0; i < markersObject.usersArray.length; i++) {
    ...
    makeControlBtnsArr[0].addEventListener("click", () => {
      ...
      finishBtn.addEventListener("click", () => {
        for (let i of deleteTarget) {
          ...
          xhr.open("POST", `http://13.124.220.4:2080/mapDelete`);
  ...
  async function publicTalkAsync() {
    let publicTalkList = await fetch(
      "http://13.124.220.4:2080/publicChatLocation"
    )
      .then((res) => {
        return res.json();
      })
      .then((result) => {
        ...
        for (let i of result) {
          const positionNow = new kakao.maps.LatLng(
            i.split("&")[0],
            i.split("&")[1]
          ); //인포윈도우 표시 위치입니다
          ...
          let callback = function (result, status) {
            if (status === kakao.maps.services.Status.OK) {
              ...
              markersObject["appendPublicChat"] = [
                "!public!" + i,
                customOverlay,
                roomNameArr[roomNameArr.length - 2] +
                  " " +
                  roomNameArr[roomNameArr.length - 1],
              ];
              let publicChatRoomList = createUserOrgchat(
                positionNow,
                roomNameArr[roomNameArr.length - 2] +
                  " " +
                  roomNameArr[roomNameArr.length - 1],
...
function createOverlay(id, mapNow, markerNow, lat, lng, time) {
  ...
  let followRequestURL = "http://13.124.220.4:2080/followRequest";
  let followRequestMessage = "팔로우";

  if (
    markersObject.markers[id][0] === 0 ||
    markersObject.markers[id][0] === 1
  ) {
    followRequestURL = "http://13.124.220.4:2080/unFollowRequest";
