...
//팔로우 검색창 - 검색 bar
slide.children[2].appendChild(
  tagCreate("input", { name: "followSearch", type: "text" })
);
...
styleCreate(
  slide.children[2].children[1],
  dangMapStyle.menuMapSlideSearchButton
);
...
slide.children[2].children[1].addEventListener("click", function () {
  ...
  xhr.open("POST", `http://13.124.220.4:2080/followSearch`, true);
  ...
  xhr.addEventListener("load", function () {
    ...
    styleCreate(
      slide.children[3].children[0],
      dangMapStyle.menuMapSlideSearchResultList
    );
  });
});
...
slide.children[1].children[0].addEventListener("mousedown", function (e) {
    ...
    if (mDown) {
      ...
      if (deltaX > 0) {
        if (marginLeftNumValue > 0) {
          ...
          marginLeftCalcValue = calculateMoveSlideValue(
            marginLeftNumValue,
            deltaX,
            widthValue
          );
  ...
  slide.children[1].children[0].addEventListener("mouseup", function () {
    mDown = false;
    changeSliderValueMarginLeft(
      slide.children[1].children[0],
      marginLeftCalcValue
    );
  });
});
...
function testFunc() {
  ...
  if (controlToggle) {
    console.log("조건문 진입");
    slide.children[slide.children.length - 1].children[2].addEventListener(
      "click",
      () => {
