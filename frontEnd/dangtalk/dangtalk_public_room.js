function main(){
  ...
  btmMeun(rootChild[5])
  // 이전채팅 불러오기
  fetch('http://15.165.220.45:2080/loadBeforeChatRequest', {
    method: 'POST',
    body:  roomCode
  ...
  function createUserOrgchat(mother,roomCode){
    ...
    exitChatRoom.addEventListener('click',()=>{
      fetch('http://15.165.220.45:2080/getOutOfPublicRoom',{
        method:'POST',
    ...
    const url = `http://15.165.220.45:2080/mapChatList`;
  ...
  //채팅
  let chat = io('http://15.165.220.45:2080/chat')
  if(firsttime==='true'){
  ...
  window.addEventListener('load',()=>{
    ...
    setTimeout(()=>{
      fetch("http://15.165.220.45:2080/bottomMenuUnreadCircle", {
        method: "POST",
        body: jwt,
      })
        .then((response) => response.json())
