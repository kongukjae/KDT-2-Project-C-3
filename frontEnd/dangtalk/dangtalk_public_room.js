function main() {
  ...
  styleCreate(
    inputWrapChild[1],
    dangtalkChattingRoomStyle.chattingSubmitButton
  );
  ...
  btmMeun(rootChild[5]);
  // 이전채팅 불러오기
  fetch("http://13.124.220.4:2080/loadBeforeChatRequest", {
    method: "POST",
    body: roomCode,
  })
  ...
  function createUserOrgchat(mother, roomCode) {
    ...
    exitChatRoom.addEventListener("click", () => {
      fetch("http://13.124.220.4:2080/getOutOfPublicRoom", {
        method: "POST",
  ...
  //채팅
  let chat = io("http://13.124.220.4:2080/chat");
  if (firsttime === "true") {
