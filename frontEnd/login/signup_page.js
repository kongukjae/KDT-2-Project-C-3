function main() {
  ...
  for (let i = 1; i < 7; i++) {
    let validity = tagCreate("div", {
      innerText: "❌입력값이 유효하지 않습니다",
    });
  ...
  dupCheck.addEventListener("click", () => {
    ...
    let _URL = `http://13.124.220.4:2080/dupCheck?id=${checkName.value}`;
    xhr.open("GET", _URL);
    xhr.send();
  ...
  function idChecker(value) {
    ...
    let validityText = "";
    if (
      value.length < asciiObject.length.min ||
      value.length > asciiObject.length.max
    ) {
      validityFlag = false;
    ...
    for (let i = 0; i < value.length; i++) {
      ...
      if (
        !(
          (target >= asciiObject.number.min &&
            target <= asciiObject.number.max) ||
          (target >= asciiObject.upperCase.min &&
            target <= asciiObject.upperCase.max) ||
          (target >= asciiObject.lowerCase.min &&
            target <= asciiObject.lowerCase.max)
        )
      ) {
        specialCheck = false;
        break;
  ...
  rootChild[6].children[0].addEventListener("click", (event) => {
    let dogGenderCheckerResult = selectorChecker(
      rootChild[6].children[0].value
    );
