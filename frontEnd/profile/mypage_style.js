function myPage() {
  ...
  function uploadImage() {
    ...
    submitbutton.addEventListener("click", () => {
      ...
      fetch("http://13.124.220.4:2080/uploadImage", {
        method: "POST",
        body: imageFormData,
      })
        .then((res) => res)
  ...
  function calendar(now) {
    ...
    xhr.open(
      "GET",
      `http://13.124.220.4:2080/allloadMap?id=${cookieId}?first=${firstDayOfMonth}?last=${lastDayOfMonth}`
    );
    xhr.send();
    xhr.addEventListener("load", function () {
  ...
  tabNames.forEach((tabName, index) => {
    ...
    tab.addEventListener("click", () => {
      ...
      if (index === 0) {
        ...
        xhrr.open("post", `http://13.124.220.4:2080/thirdmyWrite`);
        ...
        tabContents[index].innerHTML = "";
        const xhrr = new XMLHttpRequest();
        xhrr.open("post", `http://13.124.220.4:2080/secondmyWrite`);
        xhrr.setRequestHeader("Content-Type", "application/json");
        xhrr.send(`userID=${userID}`);
        xhrr.onreadystatechange = function () {
          if (this.readyState === 4 && this.status === 200) {
            ...
            if (data.length === 0) {
              ...
              for (let i = 0; i < data.length; i++) {
                ...
                postDetail.href = `http://13.124.220.4:2080/secondHandPost?nth=${i}`;
        ...
        tabContents[index].innerHTML = "";
        const xhrr = new XMLHttpRequest();
        xhrr.open("post", `http://13.124.220.4:2080/firstmyWrite`);
        xhrr.setRequestHeader("Content-Type", "application/json");
        xhrr.send(`userID=${userID}`);
        xhrr.onreadystatechange = function () {
          if (this.readyState === 4 && this.status === 200) {
  ...
  function userInfoUpdate() {
    ...
    infoChild[5].children[0].addEventListener("click", () => {
      fetch(`http://13.124.220.4:2080/userinfoUpdate`, {
        method: "POST",
        body: JSON.stringify({
          jwt: token,
