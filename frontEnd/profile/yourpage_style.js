function yourPage(){
  ...
  let _URL = `http://43.201.52.54:2080/followCheck`;
  let followRequestURL = 'http://43.201.52.54:2080/followRequest'
  ...
  followCheckXhr.addEventListener("load",()=>{
    if(followCheckXhr.response === 'yes'){
      ...
      followRequestURL = 'http://43.201.52.54:2080/unFollowRequest'
  ...
  rootChild[3].children[1].addEventListener('click',()=>{
    ...
    fetch('http://43.201.52.54:2080/createChatRoomRequest', {
      method: 'POST',
  ...
  tempXhr.open("POST", "http://43.201.52.54:2080/temperature");
  ...
  function temperatureCheck(target, sibling) {
    ...
    if(target === rootChild[4].children[1].children[0]) {
      _URL = `http://43.201.52.54:2080/UpTemp`;
      _comment = '추천';
      target.style.opacity = '1';
      sibling.style.opacity = '0.7';
    } else {
      _URL = `http://43.201.52.54:2080/DownTemp`;
      _comment = '비추천';
      target.style.opacity = '1';
      sibling.style.opacity = '0.7';
    }
...
//! 마이페이지 캘린더 부분
  ...
  function calendar(now) {
    ...
    xhr.open(
      "GET",
      `http://43.201.52.54:2080/allloadMap?id=${targetIdFromServer}?first=${firstDayOfMonth}?last=${lastDayOfMonth}`
    );
    xhr.send();
    xhr.addEventListener("load", function () {
...
//*-------------------------------
  ...
  //   window.location = "http://43.201.52.54:2080/map"
  // })
}
yourPage()
