function yourPage() {
  ...
  let _URL = `http://13.124.220.4:2080/followCheck`;
  ...
  tempXhr.open("POST", "http://13.124.220.4:2080/temperature");
  ...
  tempXhr.addEventListener("load", () => {
    ...
    if (result.re) {
      ...
      rootChild[4].children[0].children[1].innerText = `${result.temp.toFixed(
        1
      )} ℃`;
      ...
      rootChild[4].children[1].children[0].addEventListener("click", () => {
        console.log("추천함");
        temperatureCheck(
          rootChild[4].children[1].children[0],
          rootChild[4].children[1].children[1]
        );
      });
  ...
  function temperatureCheck(target, sibling) {
    ...
    if (target === rootChild[4].children[1].children[0]) {
      _URL = `http://13.124.220.4:2080/UpTemp`;
      _comment = "추천";
      target.style.opacity = "1";
      sibling.style.opacity = "0.7";
    } else {
      _URL = `http://13.124.220.4:2080/DownTemp`;
      _comment = "비추천";
      target.style.opacity = "1";
      sibling.style.opacity = "0.7";
    }
    ...
    xhr.addEventListener("load", () => {
      ...
      if (!result) {
        ...
        rootChild[4].children[0].children[1].innerText = `${result.toFixed(
          1
        )} ℃`;
        location.reload();
      }
  ...
  function calendar(now) {
    ...
    xhr.open(
      "GET",
      `http://13.124.220.4:2080/allloadMap?id=${targetIdFromServer}?first=${firstDayOfMonth}?last=${lastDayOfMonth}`
    );
    xhr.send();
    xhr.addEventListener("load", function () {
  ...
  //   window.location = "http://13.124.220.4:2080/map"
  // })
}
yourPage();
