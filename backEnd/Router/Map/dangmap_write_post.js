...
export default function callPostDangMap(request, response) {
  if (request.url.startsWith("/menuMap")) {
    ...
    request.on("end", function () {
      ...
      for (const key in cooData) {
        ...
        publicChatGenerator(
          cooData[key][2],
          cooData[key][0].toFixed(13),
          cooData[key][1].toFixed(13)
        );
        let conn = mysql.createConnection(cmServer.mysqlInfo);
        conn.connect();
        conn.query(
          `insert into map_tables(latitude, longitude, id, addData) values(${cooData[0][0].toFixed(
            13
          )}, ${cooData[0][1].toFixed(13)}, '${
            cooData[0][2].split(";")[0]
          }', now())`,
          function (err) {
            if (err) throw err;
  ...
  if (request.url.startsWith("/dragMarker")) {
    // 댕맵에서 마커 드래그가 끝났을 때
    let body = "";
    let dragData;
    ...
    request.on("end", function () {
      ...
      for (const key in dragData) {
        ...
        conn.query(
          `UPDATE map_tables SET latitude = '${dragData[key][0]}', longitude = '${dragData[key][1]}' WHERE id = '${dragData[key][2]}' and latitude = '${dragData[key][3]}' and longitude = '${dragData[key][4]}'`,
          function (err) {
            if (err) throw err;
    ...
    request.on("end", function () {
      ...
      let answer =
        result[0].replace(".", "-") +
        result[1].replace(".", "-").padStart(3, "0") +
        result[2].replace(".", " ").padStart(3, "0");
      let noon = 0;
      ...
      answer +=
        (Number(time[0]) + noon).toString().padStart(2, "0") +
        ":" +
        time[1].padStart(2, "0") +
        ":" +
        time[2].padStart(2, "0");
      ...
      console.log(answer);
      connection.query(
        `DELETE FROM map_tables WHERE id = '${id}' AND addData = '${answer}'`,
