...
export default function postCommentInput(request, response) {
  if (request.url.startsWith("/commentSubmit")) {
    ...
    request.on("end", function () {
      ...
      conn.query(
        `select post_id from dangstar where post_index = ${post_index}`,
        (err, data) => {
          if (err) throw err;
          else {
            ...
            conn.query(
              `select cm_index from cm_post order by cm_index desc limit 1`,
              (err, data) => {
                if (err) throw err;
                else {
                  ...
                  if (postID !== cm_id) {
                    conn.query(
                      `insert into alarm(id, comment, comment_index, alarm_type) values ('${postID}', '${cm_id}', '${post_index}', 'comment')`
                    );
                  }
                  conn.end();
                }
