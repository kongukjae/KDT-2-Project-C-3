...
export default function callPostLogin(request, response) {
  ...
  if (request.url.startsWith("/login")) {
          ...
          if (data.length > 0) {
            ...
            if (userLoginId === dataId) {
              ...
              if (hashPassword === dataPw) {
                ...
                response.write(`success&cookie=${idCookie}; jwt=${newjwt}`);
                // response.write(`<script>document.cookie ="jwt=${newjwt}"</script>`);
                // response.write("<script>window.location='/main'</script>");
                response.end();
              } else {
    ...
    request.on("end", function () {
      ...
      connection.connect();
      connection.query(
        `SELECT question, answer from userinfo where id = '${userinfo.id}'`,
        (error, data, fields) => {
          if (error) throw error;
          else {
            response.writeHead(200);
            if (data.length === 0) {
              ...
              parseInt(userinfo.question) === data[0].question &&
              userinfo.answer === data[0].answer
            ) {
              console.log("정보 일치");
              response.write(
                JSON.stringify(JWT.jwtCreate({ id: userinfo.id }))
              );
              // response.write()
            } else {
              console.log("정보 불일치");
    ...
    request.on("end", function () {
      ...
      const hashedTargetPassword = crypto
        .createHash("sha512")
        .update(targetPW)
        .digest("hex");
      let connection = mysql.createConnection(cmServer.mysqlInfo);
      connection.connect();
      connection.query(
        `UPDATE userinfo SET PW = '${hashedTargetPassword}' WHERE id = '${targetId}'`,
        (error, data, fields) => {
          if (error) throw error;
          else {
  ...
  // else if (request.url === "/successLogin") {
  //   let body = "";
  //   request.on("data", function (data) {
  //     body = body + data;
  //   });
  ...
  //     console.log(idCookie);
  //     response.write(
  //       `<script>document.cookie ="${idCookie}"</script>``<script>document.cookie ="jwt=${newjwt}"</script>`
  //     );
  //     response.write(`<script>document.cookie ="jwt=${newjwt}"</script>`);
  //     response.write("<script>window.location='/main'</script>");
  //     response.end();
