...
function topMenu(rootChild) {
  ...
  fetch("http://13.124.220.4:2080/alarmMark", {
    method: "POST",
    body: jwt,
  })
  ...
  alarmImg.addEventListener("click", () => {
    if (tg) {
      ...
      fetch("http://13.124.220.4:2080/alarmConent", {
        method: "POST",
        body: jwt,
      })
        .then((response) => response.json())
                ...
                if (value !== null && key === "public_chat") {
                  ...
                  let callback = function (result, status) {
                    if (status === kakao.maps.services.Status.OK) {
                      let resultSplit =
                        result[0].address.address_name.split(" ");
                      let roomName =
                        resultSplit[resultSplit.length - 2] +
                        " " +
                        resultSplit[resultSplit.length - 1];
                      let msgofPublicChat =
                        roomName + "동네에 단톡이 생성되었습니다";
                      ...
                      dismiss.addEventListener("click", () => {
                        ...
                        let url = "http://13.124.220.4:2080/deleteAlarm";
                        xhr.open("POST", url, true);
                        xhr.send(
                          `id=${jwt}&public_chat=${value}&type=public_chat`
                        );
                      ...
                      join.addEventListener("click", () => {
                        ...
                        async function moveToPublicChat() {
                          ...
                          await fetch(
                            `http://13.124.220.4:2080/createPublicChatRoomRequest`,
                            {
                              method: "POST",
                              body: JSON.stringify({
                                jwt: token,
                                roomCode: "!public!" + value,
                                roomName: roomName,
                              }),
                            }
                          )
